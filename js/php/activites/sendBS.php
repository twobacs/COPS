...
if (isset($_GET['idPat']))
	...
	$html=genBS($data);
	$subject=getSubject($idpat,$pdo);
	
	require_once ('/var/lib/webapp/v0.12.02.09/libraries/phpmailer/PHPMailerAutoload.php');
	require  ('/var/lib/webapp/v0.12.02.09/libraries/phpmailer/class.phpmailer.php');
	...
	$mail->SMTPSecure = 'tls';
	$mail->SMTPAuth = true;
	$mail->Username = "logistique.zp5317@gmail.com";
	$mail->Password = "Zp5317++";
	$mail->setFrom('logistique.zp5317@gmail.com', 'Plateforme COPS');
	$mail->addAddress('destinataires_bs@policemouscron.be', 'Destinataires BS');
	// $mail->addAddress('demets.philippe@policemouscron.be', 'Philippe Demets');
	// $mail->addAddress('monpays.david@policemouscron.be', 'David Monpays');
	// $mail->addAddress('sarlet.patrick@policemouscron.be', 'Patrick Sarlet');
	// $mail->addAddress('douterlungne.laurent@policemouscron.be', 'Laurent Douterlungne');
	// $mail->addAddress('debrauwere.dominique@policemouscron.be', 'Dominique Debrauwere');
	// $mail->Subject = 'Envoi d\'un mail via clic sur "Envoyer BS"';
	...
	if (!$mail->send()) 
		...
		{
		echo "Votre BS a été correctement transmis.  Vous pouvez à présent vous déconnecter.";
		}
...
function genBS($data)
	...
	$html='<!doctype html><html lang="fr"><head><link rel="stylesheet" href="178.33.227.92/var/www/templates/mytpl/css/style.css"></head><body>';
	$html.='<h2>Bulletin de service</h2>';
	$html.='Ce mail est g&eacute;n&eacute;r&eacute; automatiquement. Veuillez ne pas y r&eacute;pondre.  Les mails envoy&eacute;s &agrave; cette adresse ne sont pas lus. Merci.';
	$html.='<table>';
	$html.='<tr><td width="50%"><b>Date</b> : '.substr(datefr($data['patrouille']['debut']),0,10).'</td><td><b>Horaire</b> : '.substr(datefr($data['patrouille']['debut']),14,5).' - '.substr(datefr($data['patrouille']['fin']),14,5).'</td></tr>';
	$html.='</table>';
	$html.='<table>';
	...
	if($data['vv']['ttl']==0)
		...
		for($i=0;$i<$data['vv']['ttl'];$i++)
			{
			$html.=$data['vv'][$i].'<br /><hr>';
			$html.='
			Plein effectu&eacute; : 
			<div id="plO'.$i.'">
			<input type="radio" id="pleinO'.$i.'" value="Oui" name="plein'.$i.'" onclick="checkPlein(\''.$i.'\');">Oui</div>
			<div id="plN'.$i.'">
			<input type="radio" id="pleinN'.$i.'" value="Non" name="plein'.$i.'" onclick="checkPlein(\''.$i.'\');">Non</div>';
			
			$html.='D&eacute;g&acirc;ts &eacute;ventuels constat&eacute;s :<br />';
			$html.='<div id="degats'.$i.'"><input type="text" size="90%" name="DegatsVV'.$i.'" id="DegatsVV'.$i.'" value="N&eacute;ant" style="text-align:center;" onfocusout="DegatsVV(\''.$i.'\');"></div>';
	...
	for ($i=0;$i<$data['mission']['ttl'];$i++)
		...
		if (isset($data['mission'][$i]['code_prest']))
			...
			// if ($data['mission'][$i]['heure_fin']!='0000-00-00 00:00:00')	
				// {
			...
			// else
				...
				// $html.='<td></td>';
				// }
			$html.='<td>'.$data['mission'][$i]['code_prest'].'</td>';
			$html.='<td>'.$data['mission'][$i]['commentaire'];
			$html.=(isset($data['mission'][$i]['lieu'])) ? $data['mission'][$i]['lieu'] : '';
			$html.='</td></tr>';
			}
		}
	...
	if (($data['activite']['defAss']!=0)OR($data['activite']['defImm']!=0)OR($data['activite']['defCT']!=0)OR($data['activite']['defAssImCT']!=0)OR($data['activite']['defAssIm']!=0)OR($data['activite']['defImmCT']!=0)OR($data['activite']['defPC']!=0))
		{
	...
	if(($data['activite']['pvaAssur']!=0)OR($data['activite']['pvaPC']!=0)OR($data['activite']['pvaCI']!=0)OR($data['activite']['pvaExtTrBoite']!=0)OR($data['activite']['pvaPneus']!=0)OR($data['activite']['pvaCT']!=0)OR($data['activite']['pvaIm']!=0))	
		{
